...
private:
	...
	CSearchState *GetSearchStateByHandle( FileFindHandle_t handle )
	{
		...
		FS_Facade_ReportMissingSearchState( handle );
		return nullptr;
	}
...
public:
	...
	{
		FS_Facade_ClearSearchPath();
	}

	void AddSearchPath( const char *path, const char *id ) override
	{
		FixupPath( p, path );
		FS_Facade_AddGameDirectory( p, FS_CUSTOM_PATH );
	}

	void AddSearchPathNoWrite( const char *path, const char *id ) override
	{
		FixupPath( p, path );
		FS_Facade_AddGameDirectory( p, FS_NOWRITE_PATH | FS_CUSTOM_PATH );
	}
	...
	void RemoveFile( const char *path, const char *id ) override
	{
		...
		FS_Facade_Delete( fullpath ); // FS_Delete is aware of slashes
	}
	...
	void CreateDirHierarchy( const char *path, const char *id ) override
	{
		...
		Q_snprintf( fullpath, sizeof( fullpath ), "%s/%s", IdToDir( dir, sizeof( dir ), id ), path );
		FS_Facade_CreatePath( fullpath ); // FS_CreatePath is aware of slashes
	}

	bool FileExists( const char *path ) override
	{
		FixupPath( p, path );
		return FS_Facade_FileExists( p, false );
	}

	bool IsDirectory( const char *path ) override
	{
		FixupPath( p, path );
		return FS_Facade_SysFolderExists( p );
	}
	...
	FileHandle_t Open( const char *path, const char *mode, const char *id ) override
	{
		...
		FixupPath( p, path );
		fd = FS_Facade_Open( p, mode, IsIdGamedir( id ));
	...
	{
		FS_Facade_Close( static_cast<file_t *>( handle ));
	}
	...
	void Seek( FileHandle_t handle, int offset, FileSystemSeek_t whence ) override
	{
		...
		FS_Facade_Seek( static_cast<file_t *>( handle ), offset, whence_ );
	}

	unsigned int Tell( FileHandle_t handle ) override
	{
		return FS_Facade_Tell( static_cast<file_t *>( handle ));
	}

	unsigned int Size( FileHandle_t handle ) override
	{
		return FS_Facade_FileHandleSize( static_cast<file_t *>( handle ));
	}

	unsigned int Size( const char *path ) override
	{
		FixupPath( p, path );
		return FS_Facade_FileSize( p, false );
	}

	long int GetFileTime( const char *path ) override
	{
		FixupPath( p, path );
		return FS_Facade_FileTime( p, false );
	}
	...
	{
		return !FS_Facade_Eof( static_cast<file_t *>( handle ));
	}

	void Flush( FileHandle_t handle ) override
	{
		FS_Facade_Flush( static_cast<file_t *>( handle ));
	}
	...
	{
		return FS_Facade_Read( static_cast<file_t *>( handle ), buf, size );
	}

	int Write( const void *buf, int size, FileHandle_t handle ) override
	{
		return FS_Facade_Write( static_cast<file_t *>( handle ), buf, size );
	}

	char *ReadLine( char *buf, int size, FileHandle_t handle ) override
	{
		const int c = FS_Facade_Gets( static_cast<file_t *>( handle ), buf, size );
	...
	int FPrintf( FileHandle_t handle, char *fmt, ... ) override
	{
		...
		va_start( ap, fmt );
		ret = FS_Facade_VPrintf( static_cast<file_t *>( handle ), fmt, ap );
		va_end( ap );
	...
	const char *FindFirst( const char *pattern, FileFindHandle_t *handle, const char *id ) override
	{
		...
		FixupPath( p, pattern );
		search = FS_Facade_Search( p, true, IsIdGamedir( id ));
	...
	void FindClose( FileFindHandle_t handle ) override
	{
		...
		if( i == nullptr )
		{
			FS_Facade_ReportMissingSearchStateInFunction( __func__, handle );
			return;
		}
	...
	const char *GetLocalPath( const char *name, char *buf, int size ) override
	{
		...
		fullpath = FS_Facade_GetDiskPath( p, false );
		if( !fullpath )
			return nullptr;
	...
	bool FullPathToRelativePath( const char *path, char *out ) override
	{
		...
		return FS_Facade_FullPathToRelativePath( out, p, 512 );
	}

	bool GetCurrentDirectory( char *p, int size ) override
	{
		return FS_Facade_GetRootDirectory( p, size );
	}
	...
	bool AddPackFile( const char *path, const char *id ) override
	{
		...
		return FS_Facade_MountArchiveFullpath( fullpath, FS_CUSTOM_PATH ) != false;
	}
	...
	FileHandle_t OpenFromCacheForRead( const char *path , const char *mode, const char *id ) override
	{
		...
		return FS_Facade_OpenReadFile( p, mode, IsIdGamedir( id ));
	}
