...
xash::filesystem::SearchPathMemoryOps MakeSearchPathMemoryOps(
	const fs_runtime_searchpath_memory_ops_t *ops);
...
searchpath_t *RuntimeAllocSearchPath(void *context, bool clear)
{
	const fs_runtime_searchpath_memory_ops_t *ops =
		static_cast<const fs_runtime_searchpath_memory_ops_t *>(context);
...
void RuntimeFreeSearchPath(void *context, searchpath_t *path)
{
	const fs_runtime_searchpath_memory_ops_t *ops =
		static_cast<const fs_runtime_searchpath_memory_ops_t *>(context);
	...
	const fs_runtime_searchpath_memory_ops_t *ops)
{
	if (!ops)
	{
		xash::filesystem::SearchPathMemoryOps empty = {};
		return empty;
	}

	xash::filesystem::SearchPathMemoryOps result = {
		const_cast<fs_runtime_searchpath_memory_ops_t *>(ops),
		RuntimeAllocSearchPath,
		RuntimeFreeSearchPath
	};
	return result;
}
...
{
	return g_filesystemRuntime.allocateSearchPath(
		MakeSearchPathMemoryOps(ops), clear != 0);
}

void FS_FilesystemRuntime_FreeSearchPath(
	const fs_runtime_searchpath_memory_ops_t *ops, searchpath_t *path)
{
	g_filesystemRuntime.freeSearchPath(MakeSearchPathMemoryOps(ops), path);
}
