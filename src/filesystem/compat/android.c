...
searchpath_t *FS_AddAndroidAssets_Fullpath( const char *path, int flags )
{
...

	{
		fs_searchpath_callbacks_t callbacks = {
			FS_PrintInfo_AndroidAssets,
			FS_Close_AndroidAssets,
			FS_OpenFile_AndroidAssets,
			FS_FileTime_AndroidAssets,
			FS_FindFile_AndroidAssets,
			FS_Search_AndroidAssets,
			FS_LoadAndroidAssetsFile
		};
		...
		FS_SearchPath_Init( search, assets->package_name,
			SEARCHPATH_ANDROID_ASSETS,
			FS_NOWRITE_PATH | FS_CUSTOM_PATH,
			&callbacks );
	}
