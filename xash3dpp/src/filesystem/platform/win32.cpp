...
// Uses a two-call MultiByteToWideChar so paths of any length are handled.
...
std::int64_t read(OsFd& fd, void* buf, std::size_t size) noexcept {
    return static_cast<std::int64_t>(::_read(fd.get(), buf, static_cast<unsigned>(size)));
}

std::int64_t write(OsFd& fd, const void* buf, std::size_t size) noexcept {
    return static_cast<std::int64_t>(::_write(fd.get(), buf, static_cast<unsigned>(size)));
}
