...
// compliance-allow(thread-assert): stateless Winsock ::send() wrapper over a
...
// compliance-allow(thread-assert): stateless non-blocking Winsock ::connect()
// wrapper over a caller-owned OsSocket handle — header-contracted
// T_NetIO-ready, not a Main-pinned lifecycle step
