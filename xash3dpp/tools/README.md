...
`compliance_scan.py`, `stub_scan.py`, `markdown_lint.py`, `q21_scan.py`,
`docs_check.py`, and `census.py` writes the envelope JSON to a file (one-line summary on stdout,
exit code unchanged); `--split-by-subsystem DIR` on `compliance_scan.py` and
`census.py` additionally writes `DIR/<subsystem>.json` per subsystem. MCP:
`compliance_scan` takes `out_path` and returns a small summary when writing
to a file.
...
| `q21_scan.py` | Q-21 axis-coverage scan: every boundary doc's `## Extension axes (Q-21)` table diffed against the CURRENT `G-*`/`P-*` set parsed from `docs/design/extension-goals.md` (additive set — never hardcoded). Exit 1 on a missing axis row or missing section; `unknown_axes` informational. The mechanical pre-pass of the `extension-door-auditor` agent / `audit-extension-doors` prompt — catches the drift class that produced 56 missing rows in the 2026-07 audit | audit-extension-doors; consolidation audits; boundary-doc edits |
| `docs_check.py` | Documentation-drift scan, two families. `anchors`: every backticked `` `file.cpp:123` `` citation in `xash3dpp/docs/**` must resolve and must still point at the text recorded in `docs/.doc-anchors.json` — `--bless` records it, `--repair` rewrites line numbers whose recorded text merely MOVED (the common case, fixed for free, which is what keeps the running cost at zero). `claims`: `<!-- verify: <predicate> -->` beside a binding assertion — `grep-count(regex, glob)`, `symbol-exists(sym)`, `compliance-rule-exists(id)`, `census(sub, key)`, each optionally compared with `== >= <= > <` (default `>= 1`). Exempt one anchor with `<!-- verify-skip: reason -->`. Ambiguous basenames are informational, never findings. Exists because the 2026-07 audit found ~17 false doc claims by hand and none of them was catchable | pre-pr; finish-subsystem; any doc or boundary-spec edit |
| `census.py` | Per-subsystem ground-truth census: src TU count, `assert_thread_role` call sites/files, `compliance-allow` tallies by rule, stub markers, test liveness — the numbers boundary/threading docs quote (and hand-counted, and let rot). Doc refreshes paste from it; audits diff quoted-vs-actual. Always exits 0 (informational) | document-architecture, analyse-subsystem refreshes; audits |
| `markdown_lint.py` | Lint markdown with the repo's pymarkdownlnt config (`xash3dpp/.pymarkdown.json`); repo-relative paths. CLI twin of the MCP `markdown_lint` tool (was MCP-only) — hooks/CI can run the same gate. `xash3dpp/.markdownlint.json` (the VS Code markdownlint extension config) is kept aligned to the pymarkdown disables so editor squiggles match the repo gate | doc-touching commits; docs sweeps |
