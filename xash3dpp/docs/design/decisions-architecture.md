...
| `save-boundary` | SAV-OQ-1 (format extension door) | ✅ **landed 2026-07-20** (decided 2026-07-19 campaign B4; shipped S8.5 `912534df`, range S8.1-S8.7 `e38aa558`..`d3cc4d1e`): reserved `.HLX` side-block namespace (matches the `*.HL?` cleanup glob, invisible to legacy exact-name readers); self-describing magic+version+size; Chunk 8 shipped skip logic + foreign-block round-trip test only, no producer (door-keep, as decided) | — |
| `save-boundary` | SAV-OQ-2 (IFieldSink adoption) | ✅ **landed 2026-07-20** (decided 2026-07-19 campaign B4; shipped S8.1 `e38aa558` through S8.5 `912534df`, range S8.1-S8.7 `e38aa558`..`d3cc4d1e`): adopted — writer split behind `IFieldSink` (engine-side framing seam only; game-DLL field codec untouched); load path structured as pure parse helpers, independently reused by `SV_GetSaveComment`'s standalone hand-parse | — |
| `save-boundary` | SAV-OQ-3 (FIELD_FUNCTION reverse symbol lookup) | ✅ **landed 2026-07-20** (decided 2026-07-19 campaign B4; platform primitive `cce2ac5b`, save glue S8.7 `90455c98`/`d3cc4d1e`, range S8.1-S8.7 `e38aa558`..`d3cc4d1e`): platform owns the dynlib reverse-lookup primitive (`name_for_symbol` / export enumeration); save owns only the TYPEDESCRIPTION glue + per-DLL ordinal cache | — |
...
| `sound-boundary` | SND-OQ-2 (quiesce/drain + wavdata epoch + shutdown order) | ✅ **resolved 2026-07-20** (S9.7b topology slice, ratified after the four-dimension gate): (a) **fence** — `AudioTopology::flush()` submits a `FlushEpoch` command on the reserved lane and waits for `acked_epoch_ >= e`; the decoder acks with a release store when it POPS the command, and because the MPSC is strictly FIFO the ack proves every earlier command was applied. `flush()` returns `bool` and is `[[nodiscard]]`: `false` (submit refused or spin bound exhausted) means the caller must NOT treat prior audio as quiesced. (b) **ownership** — decoded `AudioData` is owned by `SfxRegistry` on T_Main; the decoder holds only borrowed `const AudioData*`. The gate found that per-word `release()` made those borrows unsound, so cache entries are now **retained for the registry's lifetime** (address stability comes from the constructor's `reserve(sound_max_sfx)` plus the MAX_SFX refusal, mirroring legacy's fixed `s_knownSfx[MAX_SFX]`); see the sound-boundary deviation entry. (c) **shutdown** — stop accepting → device `set_active(false)` + detach fill source → spin until no callback is executing → join pump → join decoder (running a final drain so nothing submitted before the close is lost) → destroy ring/queue/scratch. Teardown therefore rests on **join**, the strongest form of the guarantee; the epoch fence covers live sfx retirement only. | impl |
| `sound-boundary` | SND-OQ-3 (audio MPSC queue-full policy) | **decided 2026-07-19** (campaign B3), **amended 2026-07-20** (S9.7b, after the gate flagged code/register drift): reserved-capacity STOP/CHANGE fast lane — STOP-class commands (STOP/CHANGE_VOL/CHANGE_PITCH, `AlterChannel`, `StopAllSounds`, `FlushEpoch`) and the per-frame control message can never be refused by a START-full queue. **Amendment**: START overflow blocks the producer for a bounded spin and then **drops, counted in `SoundStats::dropped_sounds`** — it does NOT assert. The original "asserted in debug" clause is withdrawn: a hard assert on producer-side saturation converts a merely stalled decoder into a process abort, and it makes the guarantee untestable (the saturation test must be able to observe the refusal). An unbounded block was likewise rejected — it would stall the frame on T_Main. A dropped START behind a 128-deep unserviced backlog is the least-audible failure and is visible in always-on stats, which is the boundary's own stated rationale for the fast lane. | scaffold |
...
| `sound-boundary` | SND-OQ-6 (DSP `idsp_room == 29` off-by-one) | ✅ **decided 2026-07-20** (S9.5 room-DSP slice): REPRODUCE-WITH-DEFINED-BEHAVIOUR — both preset tables (`rgsxpre`/`rgsxpre_hlalpha052`) padded to a 30th row (index `k_max_room_types`==29) with an all-zero `RoomPreset`; the clamp bound (`k_max_room_types`==29) is kept textually distinct from the padded storage size so `bound(0, idsp_room, MAX_ROOM_TYPES)` stays byte-for-byte identical to legacy (index 29 IS reachable, preserved not fixed). A zeroed row was chosen over any "recovered" OOB content because legacy's actual out-of-bounds read is build/compiler/optimisation-dependent, not a reproducible value; the zeroed row is behaviourally indistinguishable from preset 0 ("off") for every field that affects observable mix output (pinned in `test_sound_dsp.cpp`'s `test_snd_oq6_index29_behaves_like_off`). Implemented in `xash3dpp/include/xash3dpp/private/sound/dsp.hpp` (file header) + `src/sound/dsp.cpp`. | — |
