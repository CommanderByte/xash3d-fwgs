...
| 11 — physics | Per-body `PhysicsContext`, no new global pmove state (threading-model §8.2) — **binding**, this is G-2's physics door. *2026-07-20 audit rider*: `PhysicsContext` must take the sim clock **from the runtime it is constructed against, never as a copied scalar** — `EngineBridge::sv_time` is the worked example of what the copy costs (read at four pfn slots, written nowhere, silently disabling legacy's edict slot-reuse grace; fixed `6dfed16f`) |
| 12 — client | *Precondition corrected 2026-07-20*: the thread-model decision here is **network I/O only** — §3.6 already assigns the render thread to Chunk 13, so the old bundled wording was undischargeable. The standing default is §3.5 ("Model B unchanged"). The cvar `shared_mutex` retrofit is **not** forced by NetIO (networking reads zero cvars) and §8.3's "add a shared_mutex" is wrong as written — the read path returns pointers the write path frees. First P-1 consumer is now named with a file:line: the NetIO→sim datagram handoff |
| 13 — renderer | `RenderFrame` is the P-2 reference implementation **and is zero code** — build it or stop citing it (`published-snapshot-brief.md`). The overlay is G-4's first visual frontend. This chunk also owns the render-thread decision (Q-6/Q-10) and is imagelib's **only** consumer, so its compressed-format contract gets exercised here for the first time |
| **Audit riders (2026-07-20)** | Three door-class briefs delivered: **HB-5** verdict is *no primitive warranted* — one cross-thread publisher, zero consumers; ships a vocabulary rule instead. **HB-6** has a buildable slice with an in-tree consumer, but §6.2's positional aggregator is *unbuildable* (host links neither sound nor input nor content nor imagelib). **HB-7** is **not** a G-5 precondition — all three shortlisted runtimes bridge today; AngelScript's gap is per-VM attribution, not alignment |
