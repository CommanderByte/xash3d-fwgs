...
> Refreshed 2026-07-06 (as-built pass). Re-verified against the shipped
> `src/networking/**`: `compliance_scan.py networking` is **clean**; the tree
> carries **55 `compliance-allow(thread-assert)`** annotations and **zero
> `assert_thread_role`** call sites — enforcement is annotation + documentation
> only, exactly as described below. This folder doc and the inline
> `## Threading` section of `docs/boundaries/networking-boundary.md` share one
> fact base (55/0) and were refreshed together; the boundary also carries the
> **T_NetIO flip table** (which of the 55 move to a runtime assert when G-2
> lands) under its `## Extension axes (Q-21)` section — read the two together.
>
> **As-built confirmations this pass:** (a) `NetAddress::to_string` writes a
> caller-owned `std::span<char>` — the legacy `NET_AdrToString` static return
> buffer is gone, so no `Race-static-buf` exists here. (b) `qport` is
> **caller-supplied** (`NetchanConfig::qport`), not generated by a networking
> RNG — the legacy `Netchan_Init` random-qport global is gone, so there is
> **no RNG / no random-state** anywhere in the subsystem (`LagQueue` takes the
> caller's drop decision; it holds no generator). (c) No info-string/userinfo
> key parser exists yet (the `net_api_t` `ValueForKey` family is deferred), so
> the static-buffer key-parse hazard those functions carry in legacy is
> **absent**. (d) The one nuance to preserve at flip time: `DeltaTables` is
> single-thread by contract on the **sim** thread (server/client), *not*
> NetIO — see the boundary flip table.
