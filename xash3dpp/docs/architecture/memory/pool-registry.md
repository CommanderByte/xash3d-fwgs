...
| Field | Type | Role |
|-------|------|------|
| `pool_index` | `uint32_t` | 1-based `PoolHandle::index`; 0 means untagged/`k_null_pool` |
...
```cpp
enum class SlotState : uint8_t { Free = 0, Busy = 1, Active = 2 };
```
...
|------|----|-----|-------------|
| `Free` | `Busy` | `create_pool` (CAS) | `acquire` on success |
| `Busy` | `Active` | `create_pool` | `release` |
...
| Field | Type | Role |
|-------|------|------|
| `name[64]` | `char[]` | Null-terminated pool name; written during `Busy`, read after `Active` |
...
|-------|--------|-------------------|
| `System` | Implemented | `malloc` / `realloc` / `free` |
| `Arena` | Placeholder | Falls back to `System`; reserved for future bump allocator |
| `Slab` | Placeholder | Falls back to `System`; reserved for future fixed-size slab |
...
| Field publish | `release` store of `Active` | Pairs with `acquire` load by readers |
| `live_bytes` / `total_allocs` / `total_frees` | `std::atomic`, `memory_order_relaxed` | Lock-free; no inter-pool ordering needed |
| `name[64]` | Protected by `Busy`/`Active` ordering | Safe: read only after `acquire` load of `Active` |
| `do_alloc` / `do_free` / `do_realloc` / `ctx` | Protected by `Busy`/`Active` ordering | Same as `name` |
...
`destroy_pool`'s clears are visible: a reader that sees `state != Active` falls
back to `malloc`/`free` directly.

## Error handling

- `create_pool`: returns `k_null_pool` when the registry is full; does not call the
...
- `create_pool` with `AllocStrategy::Arena` or `Slab` currently falls back to
  `System` without a warning. This is intentional — future implementations will
