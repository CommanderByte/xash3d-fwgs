...
1. **`CIDirectory`** — a case-insensitive directory resolver that detects
...
1. Calls `get_or_populate(root_/subdir)` — builds a sorted entry list if not
   cached.
1. Binary-searches the list case-insensitively for `name`.
1. Returns the exact on-disk spelling, or `nullopt`.
