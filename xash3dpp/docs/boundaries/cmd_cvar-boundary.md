...
| **P-5** narrowest-state signatures | **Yes — already minimal** | Public entry points are `CmdCvarContext` methods (the context *is* the subsystem aggregate — the Q-22 orchestrator carve-out); internal helpers (`pool_dup`, tokenizer, hash map) take only the value/view they touch. No wider-than-needed signature found (2026-07-19 audit). |
...
| **P-7** pool-owned RAII lifecycle | **Yes — satisfied** | `CmdCvarContext` is a pimpl over `pool_new<Impl>` (`context.cpp`); cvars/commands/aliases and hash-map nodes are pool-backed (`mem_calloc`/`mem_alloc` + `pool_dup`); no class-scoped `operator new` in the subsystem. |
| **P-8** annotation discipline | **Yes — satisfied (denominatored)** | 2026-07-19 `annotation-coverage` scan: all marker classes at 100% for cmd_cvar; the 4 `assert_thread_role(Main)` mutator sites match the Threading section's enumeration. Keep coverage statements denominatored. |
| **G-2** game ABI v2 | Door-keep (frozen surface holder) | `cvar_t`/`convar_t` layouts + `FCVAR_*` wire semantics are the frozen game-facing surface cmd_cvar guards; a v2 ABI rebinds registration through a context-carrying descriptor at the abi shim — the dual registration paths (`cvar_register_engine`/`cvar_register_dll`) already model the split, so no cmd_cvar rework is forced. |
| **G-3** dedicated debug thread | Consumer (see P-2) | An off-main debug reader uses the `CvarDesc` snapshots + `generation` epoch, never the live registry — identical posture and door rules to the P-2 row. |
| **G-4** expanded in-game debugging | Provider via P-4 | `CvarDesc`/`CommandDesc` are the "one layer, many frontends" surface G-4 consumes (the P-4 row's door rule *is* the G-4 obligation). |
...
| `32`, `126` | `cvar_ops.cpp:172` `if (c < 32 \|\| c > 126)` | **frozen** — ASCII printable-range bounds for `FCVAR_PRINTABLEONLY` enforcement (legacy `Cvar_ValidateString` semantics); character-set constants, not tunables |
