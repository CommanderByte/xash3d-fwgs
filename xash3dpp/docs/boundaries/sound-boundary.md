...
| **P-3** context-first entry points | **Delivered (legacy defect not inherited)** | As built, every mixer/topology entry takes explicit context (`Mixer&`, `ChannelApplyContext&`, `AudioTopology&`) — no file-scope mutable mixer state was ported, and the campaign added none. The remaining honest debt is the `host.soundpool`/`sndpool` P-7 item recorded elsewhere in this section. Original recon finding, retained for provenance: the legacy mixer is built entirely on file-scope mutable state (the `snd` global, mix scratch buffers, `S_GetSoundtime` statics — R9.1). Every new mixer entry point must carry context (a `Sound&`/`MixContext&`) instead of reaching `snd`; the ABI-frozen `channels`/`raw_channels` pointers inside `snd_globals_t` are the documented P-3 exception class (forced by the SoundAPI v1 contract). soundlib's `sound` global and VOX's `rgpszrawsentence` table are the same pattern at smaller scale — see Owned state disposition above. |
...
| **G-5** scripting runtime — commands via `cmd_add` = scripted-scenario surface | **Door-keep DISCHARGED — exercised, not just reserved** | All 14 sound commands are registered through the B5 `cmd_add` context overload (`src/sound/sound.cpp`), unrestricted except `music` (`FCMD_OVERRIDABLE`) and `dsp_profile` (`FCMD_PRIVILEGED`, legacy's one restricted command). Beyond the door-keep, **S9.8 actually executes it**: `tests/sound/test_sound_witness.cpp` drives a 25-frame scripted sequence through `cbuf_add_text`/`cbuf_execute` on a real `CmdCvarContext` — the deliberate G-5 stage-a rehearsal — and pins byte-identical PCM out the far end. A scripted scenario drives sound via exactly the commands a player would type; that is now demonstrated, not asserted. |
...
| **SND-OQ-4** | Music streaming: fence `s_stream` out vs. codec vends `IAudioStream` | ✅ **DECIDED 2026-07-19 (campaign B3)** — shape (a): `s_stream.c`'s background-track logic is fenced OUT of Chunk 9 entirely, and the codec seam RESERVES an `IAudioStream` vend point (per-format open/read/seek/tell/close mirroring `streamfmt_t`) so the follow-up streaming slice is purely additive rather than a refactor. Rationale kept from the recon: the stream v-table is strictly wider than the one-shot `loadwavfmt_t`, so streaming is architecturally distinct in legacy already. The `music` command is registered with a summary body pending that slice. | resolved |
| **SND-OQ-5** | Ring payload int16 — byte-identical rationale | ✅ **DECIDED 2026-07-19 (B3/B6), IMPLEMENTED S9.7b** — `core::SpscRing<int16_t, 16384>` carrying interleaved stereo device-format frames, with no float stage anywhere between the mix kernels and the device (see the as-built table above). A float or wider intermediate would need a second clamp/convert stage and risk non-bit-exact output; the kernels already produce clamped int32 accumulators before the final `CLIP16` narrow. threading-model §5.2 was amended from its original float wording to match. | resolved |
