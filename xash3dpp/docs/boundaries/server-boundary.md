...
| **P-5** narrowest-state signatures | **Yes — orchestrator carve-outs documented** | Slice free functions take the narrowest sub-aggregate (`ServerRuntime&` slices, `playermove_t*`, `EntityView`); the whole-`ServerRuntime` signatures are the frame/lifecycle orchestrators the Q-22 carve-out names. Q-22 promotion candidates were re-checked clean 2026-07-19 (WorldLinks verdict). |
...
| **P-8** annotation discipline | **Yes — satisfied (denominatored)** | 2026-07-19 scan: lifetime 67/67, safety 36/36, thread_assert 32+2-exempt = 34/34, thread_safety 1/1; the 92-assert/26-file census verified exact by two independent audit passes. Keep denominators. |
