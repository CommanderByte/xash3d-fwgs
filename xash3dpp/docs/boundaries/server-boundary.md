...
| `IMapLoaderObserver` impl | map_loader FSM | **NOT implemented by the server** — corrected 2026-07-20; this row asserted an impl that has never existed. `MapLoader` declares the interface and the `attach_observer`/`detach_observer` seam (`map_loader.cpp:252`, `:261`), and nothing under `src/server/` implements it, so `on_load_begin`/`on_load_end` around world swaps (host boundary OQ-2) is an **unowned door**, not a shipped provision. <!-- verify: grep-count(:\s*(public\s+)?IMapLoaderObserver\b, xash3dpp/src/server/**/*.cpp) == 0 --> |
...
| Game-DLL ABI shim (`sv_game.c` tables + `src/abi/engine_funcs.cpp`) | 0-1 (lives/dies with the server; no own protocol; no extra deps) | **Same target**; source-level isolation only (`src/server/game/` + the host-boundary OQ-10 `src/abi/` extern-C surface). |
...
|------------------|----------|-------------------------------------------|
| **Q-20** edict store confinement | **Yes — headline, ✅ met** | `EdictArena` is the single ABI-exact store; `EntityView` is the zero-cost typed access seam; raw `edict_t*`/`entvars_t` access is confined to `abi/` + pmove bridge + save serializer. This is the seam G-1 reads through and G-2 rebinds behind. **Keep the confinement — it is the whole game.** No new work owed; the `entvars-confinement` compliance rule pins raw `->v.` access to those three areas. *(That rule did not exist when this row first claimed it did — Q-20 promised it "when the server scaffold lands", the scaffold landed and the rule did not. Written 2026-07-20; it passed on the first run, so the confinement had held by convention alone.)* <!-- verify: compliance-rule-exists(entvars-confinement) --> |
...
| **P-5** narrowest-state signatures | **Yes — orchestrator carve-outs documented** | Slice free functions take the narrowest sub-aggregate (`ServerRuntime&` slices, `playermove_t*`, `EntityView`). *This row claimed the whole-`ServerRuntime` signatures **are** the frame/lifecycle orchestrators the Q-22 carve-out names; that is an overclaim, corrected 2026-07-20.* The mechanical census in `modernization-opportunities/server-modernization.md:374-386` finds ~121 `(ServerRuntime &rt` definitions under `src/server/**`, of which only about ten are genuine orchestrators (`load_progs`, `unload_progs`, `deactivate_server`, `install_world_bridge`, `spawn_server`, …); 51 touch at most one member and 30 of those are pure leaves. The carve-out covers the orchestrators, not the tail — narrowing the tail is open P-5 work, and Chunk 11's entry gate asks explicitly whether `src/server/physics/` is in scope for it. |
| **P-6** services are satellites | **Yes — held** | MCP/debug/script are separate targets consuming the server's typed surfaces; the server links toward none of them. In-target satellites (query/rcon/log) stay behind their seams per the Q-11 scores. |
| **P-7** pool-owned RAII lifecycle | **Yes — conforms where it applies** | `ServerRuntime` is heap-owned via the subsystem's pool discipline; `EdictArena`/`StringPool` are Q-22 RAII classes; no class-scoped `operator new` (D2 scan clean). The frozen ABI PODs (`edict_t`, `entvars_t`) are exempt by the Q-22 retrofit guard — they stay PODs. |
| **P-8** annotation discipline | **Mostly — one real gap** | Re-derived 2026-07-20 from `census` + `compliance_scan --checks annotation-coverage`: lifetime **70/70**, thread_assert **42 annotated + 14 exempt = 56/56**, thread_safety **1/1**, safety **40/44 (90.9%)**, across **103 `assert_thread_role` sites in 27 files** and 31 src TUs. *Every figure in the previous row was wrong.* It read "lifetime 67/67, safety 36/36, thread_assert 32+2-exempt = 34/34 … 92-assert/26-file census verified exact by two independent audit passes" — the thread_assert denominator was a scanner artifact (`_MUTATOR_DEF_RX` anchored its verb alternation straight to `(`, hiding every suffixed mutator; fixed 2026-07-20), and "verified exact by two independent audit passes" verified a number the scanner could not see past. **The 4 unannotated `reinterpret_cast` sites are a genuine open gap**, not an artifact. Paste these from `census`; do not hand-count. |
