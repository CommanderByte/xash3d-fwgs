...
| **P-3** context-first, no new file-scope state | **Yes — held** | Zero mutable file-scope state in the transport stack: the legacy `net_from` / `net_message` / `net_mempool` / `net.split` globals are all gone, replaced by `NetworkContext::Impl` members and per-`Netchan` state. The only function-local static is the **const** `default_protocol_driver_registry()` Meyers singleton (Safe-RO) — it eagerly owns BOTH shipped drivers (Xash 49 + GoldSrc 48; the once-planned "deferred GoldSrc-driver singleton" never materialised as a second static — corrected 2026-07-19). A documented exception, not mutable global state. |
...
| **G-3** dedicated debug thread | Consumer (counters only) | A debug thread reads `NetworkingStats` Tier-1 atomics any-thread (P-2 row); it must never touch live transport state — after the NetIO split that state belongs to `T_NetIO`, and before it, to Main. Counters are the whole surface. |
| **G-4** expanded in-game debugging | Consumer via P-4 | Net overlays (flow, in-flight fragments) extend `stats()`/a snapshot accessor per the P-4 rule; nothing else networking-side. |
| **G-5** scripting runtime | Nothing owed now | §G-5 names no networking affordance; a tooling VM reads the same stats surface. Wire/dialect code stays byte-frozen regardless of caller (HB-2 no-touch set). |
| **P-7** pool-owned RAII lifecycle | **N/A — pimpl + members** | `NetworkContext`/`Netchan` are pimpls (`make_unique<Impl>`, the sanctioned carve-out); `PacketPool`/rings/queues are value members inside `Impl`. No pool-owned `create_<thing>` object exists in the subsystem. |
| **P-8** annotation discipline | **Yes — documents-AND-marks (denominatored)** | Every confined type carries `@thread-safety`; the **55** `compliance-allow(thread-assert)` adjudications (count verified exact vs `annotation-coverage`, 2026-07-19) each carry an inline rationale — corrected the same day to name the SIM-thread contract (`DeltaTables` is sim-confined, not NetIO; the flip table below stays authoritative for which sites become `T_NetIO` asserts). |
