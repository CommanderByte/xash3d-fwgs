...
**Recon/Boundary**: ✅ done 2026-07-04 (commit `43b07bb7`) — boundary spec `docs/boundaries/server-boundary.md`; deep dives `legacy-survey/deep-dive-server-{lifecycle,game-dll-bridge,clients,physics,world-frame,save-boundary}.md`. **Blocking OQs before scaffold**: `server-boundary#OQ-1` (PHS placement — needs a decision-register entry) and `#OQ-5` (`entvars_t` representation); see the OQ crosswalk in `decisions-architecture.md`.\
