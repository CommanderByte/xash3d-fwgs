...
### Chunk 6B — hardening retrofit (Q-22 lifecycle + QN annotation discipline)
...
**Session ladder** (one green commit per step; S2∥S3, S4∥S5, S6∥S7 may run as worktree-isolated parallel agents per the WORKFLOW multi-agent policy; S1 first — it sets the exemplars; S8/S9 serial last): S1 utilities+memory → S2 platform → S3 core → S4 filesystem → S5 cmd_cvar → S6 networking → S7 map_loader → S8 host+abi+launcher → S9 server\
**Deliverable**: every subsystem `finish_check` 10/10-capable (candidates adjudicated); `compliance_scan` all-sets clean; annotation coverage 100% with truthful exemptions; `whereami` routes 6 → 6B → 7.
