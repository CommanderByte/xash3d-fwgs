...
| T-2 | Tree-wide float/byte-**EXACT** "do-not-modernize" set | **INVARIANT** | map_loader (Q-18 trace/PVS/CRC), content (studio bone math), networking (wire bit-codec/delta widths/LZSS/OOB), server (rotated-brush ULP `clip.cpp:196`), utilities (double-precision studio math) | `modernization-opportunities/{map_loader,networking,content,server}-modernization.md`, deep-dive-{trace-pvs,delta-encoder,content}.md | Five subsystems independently flagged the same prohibition (no FMA/reassoc/ranges rewrite). Name the union in one authoritative place so no future "modernization" touches it. |
...
| T-12 | RNG unification | **HOUSE — ✅ closed Chunk 11** | core + host ownership; canonical server/sound callback injection | `boundaries/{core,physics,server}-boundary.md`, `implementation-plan.md` | One `EngineContext`-owned `LegacyRandom`, differentially gated by a byte-verified compiled C oracle; sound schedule parity remains Chunk 12. |
