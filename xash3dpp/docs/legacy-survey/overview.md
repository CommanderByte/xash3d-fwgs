...
| T-2 | Tree-wide float/byte-**EXACT** "do-not-modernize" set | **INVARIANT** | map_loader (Q-18 trace/PVS/CRC), content (studio bone math), networking (wire bit-codec/delta widths/LZSS/OOB), server (rotated-brush ULP `clip.cpp:219`), utilities (double-precision studio math) | `modernization-opportunities/{map_loader,networking,content,server}-modernization.md`, deep-dive-{trace-pvs,delta-encoder,content}.md | Five subsystems independently flagged the same prohibition (no FMA/reassoc/ranges rewrite). Name the union in one authoritative place so no future "modernization" touches it. |
