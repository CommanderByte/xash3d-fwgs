...
| sound_api.h:181-194 | `sound_interface_t` (client→engine, versioned, 7 slots): pfnS_Init/Shutdown/UpdateSound/PaintChannels/UpdateChannel/UpdateRawChannel/Spatialize/FreeSound; ch=NULL sentinel documented for both Update* | `void (*pfnS_UpdateChannel)( int ch_idx, const channel_t *ch, sound_t handle ); // ch=NULL -> channel freed` | high |
